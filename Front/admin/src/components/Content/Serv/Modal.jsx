...
import { ComboBox } from "../../Elements/Select";
...
export default function BasicModal({ pModal, pSetModal }) {
  ...
  useEffect(() => {
    if (pModal.active) {
      (async () => {
        const x = await act_getOneService(pModal.id);
        console.log("MODAL", x);
        setInputs({
          ...inputs,
          n_name: x.name,
        });
        setComboBox(x.categories[0]);
      })();
    }
  }, [pModal]);
    ...
    >
      <Box sx={style}>
        <MyForm className="tblCat-r1" onSubmit={mOnSubmit}>
          ...
          <ComboBox category={comboBox} setCategory={setComboBox}/>
          <TextField
            variant="standard"
            label="Name"
            name="n_name"
